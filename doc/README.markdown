...
### Get CloudI Running
    ...
    $ wget http://sourceforge.net/projects/cloudi/files/latest/download -O cloudi-1.2.5.tar.gz
    $ tar zxvf cloudi-1.2.5.tar.gz
    $ cd cloudi-1.2.5/src
    $ ./configure
    $ make
    $ sudo make install
...
### Create an Internal (Erlang) CloudI Service
    ...
    cloudi_service_terminate(_, #state{}) ->
        ok.
    EOF
    $ erlc -pz /usr/local/lib/cloudi-1.2.5/lib/cloudi_core-1.2.5 -pz /usr/local/lib/cloudi-1.2.5/lib/cloudi_core-1.2.5/ebin hello_world.erl
...
### Use an External (Python) CloudI Service
    ...
    import sys
    sys.path.append('/usr/local/lib/cloudi-1.2.5/api/python/')
