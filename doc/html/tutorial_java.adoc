...
The table above shows how the HTTP method becomes a suffix on the URL path to create the CloudI service name pattern suffix (cloudi_service_http_cowboy1 adds the HTTP method as a suffix in lowercase when sending the HTTP request data as a CloudI service request).
...
The book recommendation service initialization is shown below (from link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.8.0/src/main/java/org/cloudi/examples/tutorial/Service.java#L90-L140[Service.java:90-140^]):
...
CloudI includes two CloudI services that are HTTP Servers: cloudi_service_http_cowboy1 and cloudi_service_http_elli.
The book recommendation service will use cloudi_service_http_cowboy1 for handling both basic HTTP requests and WebSocket requests.
...
An example of a service request handling function is below (using the "itemRefresh" function from link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.8.0/src/main/java/org/cloudi/examples/tutorial/Service.java#L163-L209[Service.java:163-209^]):
...
Later Java versions are not supported by the Lenskit dependency but are supported by the Java CloudI API.
...
[[{prefix, "/tutorial/java/service/"},
  ...
  {args, "-Dfile.encoding=UTF-8 "
         ...
         "-ea:org.cloudi... "
         "-Xms3g -Xmx3g "
         "-jar $PWD/target/cloudi_tutorial_java-1.8.0-jar-with-dependencies.jar "
...
Now the tutorial services can be started by relying on the default CloudI configuration having cloudi_service_http_cowboy1 running on port 6464 so that CloudI Service API requests can be made dynamically (using cloudi_service_api_requests) to start new service instances:
...
The result will be similar to `["ServiceId1","ServiceId2"]` where `ServiceId1` is a UUID (Universally Unique Identifier) for the new service instance of cloudi_service_filesystem and `ServiceId2` is a UUID for the new service instance of cloudi_service_http_cowboy1.
...
. The Lenskit recommendation model has not yet been updated based on other user ratings to create suggestions so click the "Refresh Recommendation Model" button (this update action is typically done weekly or daily with online services that have many users, when Lenskit is used in production).
...
The table above shows the service subscriptions that occur during service initialization (in link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.8.0/src/main/java/org/cloudi/examples/tutorial/Service.java#L90-L140[Service.java:90-140^]).
...
When a HTTP request (on port 8080) is made to the REST API through cloudi_service_http_cowboy1, a CloudI service request is sent based on the HTTP request URL path using the default timeout that was configured for cloudi_service_http_cowboy1 service requests (10 minutes).
The Javascript interface relies on a WebSocket connection for any usage of the REST API as shown below (link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.8.0/html/tutorial/java/index.html#L542-L550[index.html:542-550^]):
...
This means that the URL path includes the Java book recommendation service prefix of "/tutorial/java/service/" with a suffix of "client/get" (all WebSocket connections are initialized with the GET HTTP method and cloudi_service_http_cowboy1 is adding the "/get" suffix to the URL path before sending a CloudI service request).
...
The service request handler function "client" is shown below (from link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.8.0/src/main/java/org/cloudi/examples/tutorial/Service.java#L429-L552[Service.java:429-552^]):
...
For scheduling an interrupt of the `poll` function the timeout parameter is used as shown below (from link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.8.0/src/main/java/org/cloudi/examples/tutorial/Service.java#L141-L146[Service.java:141-146^]):
...
Each interrupt of the `poll` function allows execution to check if it can run a chunk of function objects in the `ServiceIdle` class as shown below (from link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.8.0/src/main/java/org/cloudi/examples/tutorial/ServiceIdle.java#L9-L73[ServiceIdle.java:9-73^]):
