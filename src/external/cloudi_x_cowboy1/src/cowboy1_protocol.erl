...
-record(state, {
	...
	compress :: boolean(),
	env :: cowboy1_middleware:env(),
	onrequest :: undefined | cowboy1:onrequest_fun(),
	onresponse = undefined :: undefined | cowboy1:onresponse_fun(),
	max_empty_lines :: non_neg_integer(),
...
-include_lib("cowlib1/include/cow1_inline.hrl").

%% API.

-spec start_link(ranch1:ref(), inet:socket(), module(), opts()) -> {ok, pid()}.
...
-spec init(ranch1:ref(), inet:socket(), module(), opts()) -> ok.
...
%% Same code as cow1_http:parse_fullhost/1, but inline because we
...
request(Buffer, State=#state{socket=Socket, transport=Transport,
	...
	case Transport:peername(Socket) of
		{ok, Peer} ->
			Req = cowboy1_req:new(Socket, Transport, Peer, Method, Path,
...
-spec onrequest(cowboy1_req:req(), #state{}) -> ok.
...
-spec execute(cowboy1_req:req(), #state{}) -> ok.
execute(Req, State=#state{middlewares=Middlewares, env=Env}) ->
	execute(Req, State, Env, Middlewares).

-spec execute(cowboy1_req:req(), #state{}, cowboy1_middleware:env(), [module()])
	-> ok.
...
-spec resume(#state{}, cowboy1_middleware:env(), [module()],
...
-spec next_request(cowboy1_req:req(), #state{}, any()) -> ok.
next_request(Req, State=#state{req_keepalive=Keepalive, timeout=Timeout},
	...
	case cowboy1_req:get(connection, Req) of
		close ->
			terminate(State);
		_ ->
			%% Skip the body if it is reasonably sized. Close otherwise.
			Buffer = case cowboy1_req:body(Req) of
				{ok, _, Req2} -> cowboy1_req:get(buffer, Req2);
				_ -> close
			end,
			%% Flush the resp_sent message before moving on.
			if HandlerRes =:= ok, Buffer =/= close ->
					receive {cowboy1_req, resp_sent} -> ok after 0 -> ok end,
...
-spec error_terminate(cowboy1:http_status(), #state{}) -> ok.
error_terminate(Status, State=#state{socket=Socket, transport=Transport,
		compress=Compress, onresponse=OnResponse}) ->
	error_terminate(Status, cowboy1_req:new(Socket, Transport,
		undefined, <<"GET">>, <<>>, <<>>, 'HTTP/1.1', [], <<>>,
		undefined, <<>>, false, Compress, OnResponse), State).

-spec error_terminate(cowboy1:http_status(), cowboy1_req:req(), #state{}) -> ok.
error_terminate(Status, Req, State) ->
	_ = cowboy1_req:reply(Status, Req),
	terminate(State).
