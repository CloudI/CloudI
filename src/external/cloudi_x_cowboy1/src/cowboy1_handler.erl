...
-module(cowboy1_handler).
-behaviour(cowboy1_middleware).
...
handler_init(Req, State, Handler, HandlerOpts) ->
	Transport = cowboy1_req:get(transport, Req),
...
-spec handler_handle(Req, #state{}, module(), any())
	-> {ok, Req, cowboy1_middleware:env()} when Req::cowboy1_req:req().
...
%% Almost the same code can be found in cowboy1_websocket.
	...
	receive
		{OK, Socket, Data} ->
			...
			if	NbBytes2 > Threshold ->
					...
					exit(normal);
				true ->
					Req2 = cowboy1_req:append_buffer(Data, Req),
					State2 = handler_loop_timeout(State#state{
		...
		Message ->
			...
			Req2 = receive {OK, Socket, Data} ->
				cowboy1_req:append_buffer(Data, Req)
			after 0 ->
				Req
			end,
...
%% and as it is expected to be by cowboy1_protocol, right after we're done
...
-spec handler_terminate(cowboy1_req:req(), module(), any(),
