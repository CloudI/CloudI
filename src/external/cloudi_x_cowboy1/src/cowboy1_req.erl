...
-type cookie_opts() :: cow1_cookie:cookie_opts().
...
-record(http_req, {
	...
	pid = undefined :: pid(),
	method = <<"GET">> :: binary(),
	version = 'HTTP/1.1' :: cowboy1:http_version(),
	peer = undefined :: undefined | {inet:ip_address(), inet:port_number()},
	host = undefined :: undefined | binary(),
	host_info = undefined :: undefined | cowboy1_router:tokens(),
	port = undefined :: undefined | inet:port_number(),
	path = undefined :: binary(),
	path_info = undefined :: undefined | cowboy1_router:tokens(),
	qs = undefined :: binary(),
	qs_vals = undefined :: undefined | list({binary(), binary() | true}),
	bindings = undefined :: undefined | cowboy1_router:bindings(),
	headers = [] :: cowboy1:http_headers(),
	p_headers = [] :: [any()],
	...
	resp_state = waiting :: locked | waiting | waiting_stream
		| chunks | stream | done,
	resp_headers = [] :: cowboy1:http_headers(),
...
-spec new(any(), module(),
	...
	cowboy1:http_version(), cowboy1:http_headers(), binary(),
	inet:port_number() | undefined, binary(), boolean(), boolean(),
	undefined | cowboy1:onresponse_fun())
	-> req().
	...
	case CanKeepalive of
		...
		true ->
			case lists:keyfind(<<"connection">>, 1, Headers) of
				...
				{_, ConnectionHeader} ->
					Tokens = cow1_http_hd:parse_connection(ConnectionHeader),
...
-spec version(Req) -> {cowboy1:http_version(), Req} when Req::req().
version(Req) ->
...
-spec host_info(Req)
	-> {cowboy1_router:tokens() | undefined, Req} when Req::req().
host_info(Req) ->
...
-spec headers(Req) -> {cowboy1:http_headers(), Req} when Req::req().
headers(Req) ->
...
parse_header(Name = <<"accept">>, Req, Default) ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy1_http:list(Value, fun cowboy1_http:media_range/2)
		end);
parse_header(Name = <<"accept-charset">>, Req, Default) ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy1_http:nonempty_list(Value, fun cowboy1_http:conneg/2)
		end);
parse_header(Name = <<"accept-encoding">>, Req, Default) ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy1_http:list(Value, fun cowboy1_http:conneg/2)
		end);
parse_header(Name = <<"accept-language">>, Req, Default) ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy1_http:nonempty_list(Value, fun cowboy1_http:language_range/2)
		end);
parse_header(Name = <<"authorization">>, Req, Default) ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy1_http:token_ci(Value, fun cowboy1_http:authorization/2)
		end);
parse_header(Name = <<"content-length">>, Req, Default) ->
	parse_header(Name, Req, Default, fun cow1_http_hd:parse_content_length/1);
parse_header(Name = <<"content-type">>, Req, Default) ->
	parse_header(Name, Req, Default, fun cowboy1_http:content_type/1);
parse_header(Name = <<"cookie">>, Req, Default) ->
	parse_header(Name, Req, Default, fun cow1_cookie:parse_cookie/1);
parse_header(Name = <<"expect">>, Req, Default) ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy1_http:nonempty_list(Value, fun cowboy1_http:expectation/2)
		end);
parse_header(Name, Req, Default)
		when Name =:= <<"if-match">>;
			Name =:= <<"if-none-match">> ->
	parse_header(Name, Req, Default, fun cowboy1_http:entity_tag_match/1);
parse_header(Name, Req, Default)
		when Name =:= <<"if-modified-since">>;
			Name =:= <<"if-unmodified-since">> ->
	parse_header(Name, Req, Default, fun cowboy1_http:http_date/1);
parse_header(Name = <<"range">>, Req, Default) ->
	parse_header(Name, Req, Default, fun cowboy1_http:range/1);
parse_header(Name, Req, Default)
		when Name =:= <<"sec-websocket-protocol">>;
			Name =:= <<"x-forwarded-for">> ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy1_http:nonempty_list(Value, fun cowboy1_http:token/2)
		end);
parse_header(Name = <<"transfer-encoding">>, Req, Default) ->
	parse_header(Name, Req, Default, fun cow1_http_hd:parse_transfer_encoding/1);
%% @todo Product version.
parse_header(Name = <<"upgrade">>, Req, Default) ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy1_http:nonempty_list(Value, fun cowboy1_http:token_ci/2)
		end);
parse_header(Name = <<"sec-websocket-extensions">>, Req, Default) ->
	parse_header(Name, Req, Default, fun cowboy1_http:parameterized_tokens/1);
parse_header(Name, Req, Default) ->
...
body(Req=#http_req{body_state=waiting}, Opts) ->
	...
	CFun = case lists:keyfind(content_decode, 1, Opts) of
		false ->
			fun cowboy1_http:ce_identity/1;
		{_, CFun0} ->
			CFun0
	end,
			...
			case parse_header(<<"transfer-encoding">>, Req1) of
				{ok, [<<"chunked">>], Req2} ->
					body(Req2#http_req{body_state={stream, 0,
						fun cow1_http_te:stream_chunked/2, {0, 0}, CFun}}, Opts);
				{ok, [<<"identity">>], Req2} ->
					...
					case Len of
						...
						_ ->
							body(Req3#http_req{body_state={stream, Len,
								fun cow1_http_te:stream_identity/2, {0, Len},
								CFun}}, Opts)
					end
			end;
		{_, TFun, TState} ->
...
-spec part(Req)
	-> {ok, cow1_multipart:headers(), Req} | {done, Req}
	when Req::req().
part(Req) ->
	part(Req, [
		{length, 64000},
...
part(Buffer, Opts, Req=#http_req{multipart={Boundary, _}}) ->
	case cow1_multipart:parse_headers(Buffer, Boundary) of
		more ->
			{Data, Req2} = stream_multipart(Req, Opts),
...
part_body(Buffer, Opts, Req=#http_req{multipart={Boundary, _}}, Acc) ->
	...
	case byte_size(Acc) > ChunkLen of
		...
		false ->
			{Data, Req2} = stream_multipart(Req, Opts),
			case cow1_multipart:parse_body(<< Buffer/binary, Data/binary >>, Boundary) of
				{ok, Body} ->
...
set_resp_cookie(Name, Value, Opts, Req) ->
	Cookie = cow1_cookie:setcookie(Name, Value, Opts),
...
-spec reply(cowboy1:http_status(), Req) -> {ok, Req} when Req::req().
reply(Status, Req=#http_req{resp_body=Body}) ->
	reply(Status, [], Body, Req).

-spec reply(cowboy1:http_status(), cowboy1:http_headers(), Req)
	-> {ok, Req} when Req::req().
reply(Status, Headers, Req=#http_req{resp_body=Body}) ->
	reply(Status, Headers, Body, Req).

-spec reply(cowboy1:http_status(), cowboy1:http_headers(),
...
reply(Status, Headers, Body, Req=#http_req{
	...
	HTTP11Headers = if
		Transport =/= cowboy1_spdy, Version =:= 'HTTP/1.0', Connection =:= keepalive ->
			[{<<"connection">>, atom_to_connection(Connection)}];
		Transport =/= cowboy1_spdy, Version =:= 'HTTP/1.1', Connection =:= close ->
			[{<<"connection">>, atom_to_connection(Connection)}];
		true ->
			[]
	end,
	Req3 = case Body of
		BodyFun when is_function(BodyFun) ->
			...
			RespConn = close,
			{RespType, Req2} = if
				Transport =:= cowboy1_spdy ->
					response(Status, Headers, RespHeaders, [
						{<<"date">>, cowboy1_clock:rfc1123()},
						{<<"server">>, <<"Cowboy">>}
					], stream, Req);
				true ->
					response(Status, Headers, RespHeaders, [
...
-spec chunked_reply(cowboy1:http_status(), Req) -> {ok, Req} when Req::req().
chunked_reply(Status, Req) ->
	chunked_reply(Status, [], Req).

-spec chunked_reply(cowboy1:http_status(), cowboy1:http_headers(), Req)
	-> {ok, Req} when Req::req().
...
chunk(_Data, #http_req{method= <<"HEAD">>}) ->
	ok;
chunk(Data, #http_req{socket=Socket, transport=cowboy1_spdy,
		resp_state=chunks}) ->
	cowboy1_spdy:stream_data(Socket, Data);
...
last_chunk(Req=#http_req{socket=Socket, transport=cowboy1_spdy}) ->
	_ = cowboy1_spdy:stream_close(Socket),
	Req#http_req{resp_state=done};
last_chunk(Req=#http_req{socket=Socket, transport=Transport}) ->
	_ = Transport:send(Socket, <<"0\r\n\r\n">>),
	Req#http_req{resp_state=done}.

-spec upgrade_reply(cowboy1:http_status(), cowboy1:http_headers(), Req)
	-> {ok, Req} when Req::req().
upgrade_reply(Status, Headers, Req=#http_req{transport=Transport,
		resp_state=waiting, resp_headers=RespHeaders})
		when Transport =/= cowboy1_spdy ->
	{_, Req2} = response(Status, Headers, RespHeaders, [
		{<<"connection">>, <<"Upgrade">>}
	], <<>>, Req),
...
do_maybe_reply([
		{cow1_http_hd, _, _, _},
		{cowboy1_req, parse_header, _, _}|_], Req) ->
	cowboy1_req:reply(400, Req);
do_maybe_reply(_, Req) ->
	cowboy1_req:reply(500, Req).

-spec ensure_response(req(), cowboy1:http_status()) -> ok.
...
-spec set_bindings(cowboy1_router:tokens(), cowboy1_router:tokens(),
	cowboy1_router:bindings(), Req) -> Req when Req::req().
...
-spec chunked_response(cowboy1:http_status(), cowboy1:http_headers(), Req) ->
	{normal | hook, Req} when Req::req().
chunked_response(Status, Headers, Req=#http_req{
		transport=cowboy1_spdy, resp_state=waiting,
...
-spec response(cowboy1:http_status(), cowboy1:http_headers(),
	cowboy1:http_headers(), cowboy1:http_headers(), stream | iodata(), Req)
	-> {normal | hook, Req} when Req::req().
response(Status, Headers, RespHeaders, DefaultHeaders, Body, Req=#http_req{
	...
	ReplyType = case Req2#http_req.resp_state of
		waiting when Transport =:= cowboy1_spdy, Body =:= stream ->
			cowboy1_spdy:stream_reply(Socket, status(Status2), FullHeaders2),
			ReqPid ! {?MODULE, resp_sent},
			normal;
		waiting when Transport =:= cowboy1_spdy ->
			cowboy1_spdy:reply(Socket, status(Status2), FullHeaders2, Body),
			ReqPid ! {?MODULE, resp_sent},
			normal;
...
-spec response_connection(cowboy1:http_headers(), keepalive | close)
	-> keepalive | close.
...
response_connection([{Name, Value}|Tail], Connection) ->
	case Name of
		<<"connection">> ->
			Tokens = cow1_http_hd:parse_connection(Value),
			connection_to_atom(Tokens);
		_ ->
			response_connection(Tail, Connection)
	end.

-spec response_merge_headers(cowboy1:http_headers(), cowboy1:http_headers(),
	cowboy1:http_headers()) -> cowboy1:http_headers().
...
-spec merge_headers(cowboy1:http_headers(), cowboy1:http_headers())
	-> cowboy1:http_headers().
...
-spec status(cowboy1:http_status()) -> binary().
...
url_test() ->
	{undefined, _} =
		url(#http_req{transport=ranch1_tcp, host= <<>>, port= undefined,
			path= <<>>, qs= <<>>, pid=self()}),
	{<<"http://localhost/path">>, _ } =
		url(#http_req{transport=ranch1_tcp, host= <<"localhost">>, port=80,
			path= <<"/path">>, qs= <<>>, pid=self()}),
	{<<"http://localhost:443/path">>, _} =
		url(#http_req{transport=ranch1_tcp, host= <<"localhost">>, port=443,
			path= <<"/path">>, qs= <<>>, pid=self()}),
	{<<"http://localhost:8080/path">>, _} =
		url(#http_req{transport=ranch1_tcp, host= <<"localhost">>, port=8080,
			path= <<"/path">>, qs= <<>>, pid=self()}),
	{<<"http://localhost:8080/path?dummy=2785">>, _} =
		url(#http_req{transport=ranch1_tcp, host= <<"localhost">>, port=8080,
			path= <<"/path">>, qs= <<"dummy=2785">>, pid=self()}),
	{<<"https://localhost/path">>, _} =
		url(#http_req{transport=ranch1_ssl, host= <<"localhost">>, port=443,
			path= <<"/path">>, qs= <<>>, pid=self()}),
	{<<"https://localhost:8443/path">>, _} =
		url(#http_req{transport=ranch1_ssl, host= <<"localhost">>, port=8443,
			path= <<"/path">>, qs= <<>>, pid=self()}),
