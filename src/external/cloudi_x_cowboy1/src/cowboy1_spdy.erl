...
-type opts() :: [{env, cowboy1_middleware:env()}
	| {middlewares, [module()]}
...
-spec init(pid(), ranch1:ref(), inet:socket(), module(), opts()) -> ok.
init(Parent, Ref, Socket, Transport, Opts) ->
	process_flag(trap_exit, true),
	ok = proc_lib:init_ack(Parent, {ok, self()}),
	{ok, Peer} = Transport:peername(Socket),
	Middlewares = get_value(middlewares, Opts, [cowboy1_router, cowboy1_handler]),
	Env = [{listener, Ref}|get_value(env, Opts, [])],
	OnRequest = get_value(onrequest, Opts, undefined),
	OnResponse = get_value(onresponse, Opts, undefined),
	Zdef = cow1_spdy:deflate_init(),
	Zinf = cow1_spdy:inflate_init(),
...
parse_frame(State=#state{zinf=Zinf}, Data) ->
	case cow1_spdy:split(Data) of
		{true, Frame, Rest} ->
			P = cow1_spdy:parse(Frame, Zinf),
...
handle_frame(State=#state{socket=Socket, transport=Transport},
		{ping, PingID}) ->
	Transport:send(Socket, cow1_spdy:ping(PingID)),
	State;
...
syn_reply(#state{socket=Socket, transport=Transport, zdef=Zdef},
	...
	Transport:send(Socket, cow1_spdy:syn_reply(Zdef, StreamID, IsFin,
		Status, <<"HTTP/1.1">>, Headers)).

rst_stream(#state{socket=Socket, transport=Transport}, StreamID, Status) ->
	Transport:send(Socket, cow1_spdy:rst_stream(StreamID, Status)).

goaway(#state{socket=Socket, transport=Transport, last_streamid=LastStreamID},
		Status) ->
	Transport:send(Socket, cow1_spdy:goaway(LastStreamID, Status)).

data(#state{socket=Socket, transport=Transport}, StreamID, IsFin, Data) ->
	Transport:send(Socket, cow1_spdy:data(StreamID, IsFin, Data)).
...
data_from_file(Socket, Transport, StreamID, IoDevice) ->
	case file:read(IoDevice, 16#1fff) of
		eof ->
			_ = Transport:send(Socket, cow1_spdy:data(StreamID, true, <<>>)),
			ok;
		{ok, Data} ->
			case Transport:send(Socket, cow1_spdy:data(StreamID, false, Data)) of
				ok ->
					data_from_file(Socket, Transport, StreamID, IoDevice);
				{error, _} ->
					ok
...
-spec request_init(socket(), {inet:ip_address(), inet:port_number()},
		cowboy1:onrequest_fun(), cowboy1:onresponse_fun(),
		cowboy1_middleware:env(), [module()],
		binary(), binary(), binary(), binary(), [{binary(), binary()}])
	-> ok.
request_init(FakeSocket, Peer, OnRequest, OnResponse,
		Env, Middlewares, Method, Host, Path, Version, Headers) ->
	{Host2, Port} = cow1_http:parse_fullhost(Host),
	{Path2, Qs} = cow1_http:parse_fullpath(Path),
	Version2 = cow1_http:parse_version(Version),
	Req = cowboy1_req:new(FakeSocket, ?MODULE, Peer,
	...
	case OnRequest of
		...
		_ ->
			Req2 = OnRequest(Req),
			case cowboy1_req:get(resp_state, Req2) of
				waiting -> execute(Req2, Env, Middlewares);
				_ -> ok
			end
	end.

-spec execute(cowboy1_req:req(), cowboy1_middleware:env(), [module()])
	-> ok.
execute(Req, _, []) ->
	cowboy1_req:ensure_response(Req, 204);
execute(Req, Env, [Middleware|Tail]) ->
	case Middleware:execute(Req, Env) of
		...
		{halt, Req2} ->
			cowboy1_req:ensure_response(Req2, 204);
		{error, Status, Req2} ->
			cowboy1_req:reply(Status, Req2)
	end.

-spec resume(cowboy1_middleware:env(), [module()],
...
%% Reply functions used by cowboy1_req.

-spec reply(socket(), binary(), cowboy1:http_headers(), iodata()) -> ok.
...
-spec stream_reply(socket(), binary(), cowboy1:http_headers()) -> ok.
