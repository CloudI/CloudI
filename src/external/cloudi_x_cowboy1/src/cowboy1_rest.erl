...
%% Justin Sheehy.
-module(cowboy1_rest).
-behaviour(cowboy1_sub_protocol).

-export([upgrade/4]).

-record(state, {
	env :: cowboy1_middleware:env(),
...
-spec upgrade(Req, Env, module(), any())
	-> {ok, Req, Env} when Req::cowboy1_req:req(), Env::cowboy1_middleware:env().
upgrade(Req, Env, Handler, HandlerOpts) ->
	Method = cowboy1_req:get(method, Req),
...
method_not_allowed(Req, State, []) ->
	Req2 = cowboy1_req:set_resp_header(<<"allow">>, <<>>, Req),
	respond(Req2, State, 405);
method_not_allowed(Req, State, Methods) ->
	<< ", ", Allow/binary >> = << << ", ", M/binary >> || M <- Methods >>,
	Req2 = cowboy1_req:set_resp_header(<<"allow">>, Allow, Req),
	respond(Req2, State, 405).
...
is_authorized(Req, State) ->
	case call(Req, State, is_authorized) of
		...
		{{false, AuthHead}, Req2, HandlerState} ->
			Req3 = cowboy1_req:set_resp_header(
...
content_types_provided(Req, State) ->
	case call(Req, State, content_types_provided) of
		no_call ->
			...
			case cowboy1_req:parse_header(<<"accept">>, Req) of
				{error, badarg} ->
					respond(Req, State2, 400);
				{ok, undefined, Req2} ->
					languages_provided(
						cowboy1_req:set_meta(media_type, {<<"text">>, <<"html">>, []}, Req2),
		...
		{CTP, Req2, HandlerState} ->
			...
			case cowboy1_req:parse_header(<<"accept">>, Req2) of
				{error, badarg} ->
					respond(Req2, State2, 400);
				{ok, undefined, Req3} ->
					{PMT, _Fun} = HeadCTP = hd(CTP2),
					languages_provided(
						cowboy1_req:set_meta(media_type, PMT, Req3),
...
normalize_content_types({ContentType, Callback})
	...
	{cowboy1_http:content_type(ContentType), Callback};
...
match_media_type_params(Req, State, _Accept,
	...
	PMT = {TP, STP, Params_A},
	languages_provided(cowboy1_req:set_meta(media_type, PMT, Req),
	...
	case call(Req, State, languages_provided) of
		...
		{LP, Req2, HandlerState} ->
			...
			{ok, AcceptLanguage, Req3} =
				cowboy1_req:parse_header(<<"accept-language">>, Req2),
...
set_language(Req, State=#state{language_a=Language}) ->
	Req2 = cowboy1_req:set_resp_header(<<"content-language">>, Language, Req),
	charsets_provided(cowboy1_req:set_meta(language, Language, Req2), State).
	...
	case call(Req, State, charsets_provided) of
		...
		{CP, Req2, HandlerState} ->
			...
			{ok, AcceptCharset, Req3} =
				cowboy1_req:parse_header(<<"accept-charset">>, Req2),
...
set_content_type(Req, State=#state{
	...
	Req2 = cowboy1_req:set_resp_header(<<"content-type">>, ContentType2, Req),
	encodings_provided(cowboy1_req:set_meta(charset, Charset, Req2), State).
...
if_match_exists(Req, State) ->
	...
	case cowboy1_req:parse_header(<<"if-match">>, Req) of
		{ok, undefined, Req2} ->
...
if_match_must_not_exist(Req, State) ->
	case cowboy1_req:header(<<"if-match">>, Req) of
...
if_unmodified_since_exists(Req, State) ->
	case cowboy1_req:parse_header(<<"if-unmodified-since">>, Req) of
		{ok, undefined, Req2} ->
...
if_none_match_exists(Req, State) ->
	case cowboy1_req:parse_header(<<"if-none-match">>, Req) of
		{ok, undefined, Req2} ->
...
if_modified_since_exists(Req, State) ->
	case cowboy1_req:parse_header(<<"if-modified-since">>, Req) of
		{ok, undefined, Req2} ->
			method(Req2, State);
...
not_modified(Req, State) ->
	Req2 = cowboy1_req:delete_resp_header(<<"content-type">>, Req),
...
accept_resource(Req, State) ->
	case call(Req, State, content_types_accepted) of
		...
		{CTA, Req2, HandlerState} ->
			...
			State2 = State#state{handler_state=HandlerState},
			case cowboy1_req:parse_header(<<"content-type">>, Req2) of
				{ok, ContentType, Req3} ->
...
process_content_type(Req, State=#state{method=Method, exists=Exists}, Fun) ->
	...
	end catch ?STACKTRACE(case_clause, no_call, Stacktrace)
		error_terminate(Req, State, case_clause, no_call, Stacktrace, Fun)
	end.
...
maybe_created(Req, State) ->
	case cowboy1_req:has_resp_header(<<"location">>, Req) of
		true -> respond(Req, State, 201);
		false -> has_resp_body(Req, State)
	end.

has_resp_body(Req, State) ->
	case cowboy1_req:has_resp_body(Req) of
	...
	try last_modified(Req, State) of
		{LastModified, Req2, State2} ->
			case LastModified of
				...
				LastModified ->
					LastModifiedBin = cowboy1_clock:rfc1123(LastModified),
					Req3 = cowboy1_req:set_resp_header(
...
set_resp_body(Req, State=#state{content_type_a={_, Callback}}) ->
	try case call(Req, State, Callback) of
		...
		{Body, Req2, HandlerState2} ->
			...
			Req3 = case Body of
				{stream, StreamFun} ->
					cowboy1_req:set_resp_body_fun(StreamFun, Req2);
				{stream, Len, StreamFun} ->
					cowboy1_req:set_resp_body_fun(Len, StreamFun, Req2);
				{chunked, StreamFun} ->
					cowboy1_req:set_resp_body_fun(chunked, StreamFun, Req2);
				_Contents ->
					cowboy1_req:set_resp_body(Body, Req2)
			end,
			multiple_choices(Req3, State2)
	end catch ?STACKTRACE(case_clause, no_call, Stacktrace)
		error_terminate(Req, State, case_clause, no_call, Stacktrace, Callback)
	end.
...
set_resp_expires(Req, State) ->
	...
	case Expires of
		...
		Expires when is_binary(Expires) ->
			...
			{Req3, State2};
		Expires ->
			ExpiresBin = cowboy1_clock:rfc1123(Expires),
			Req3 = cowboy1_req:set_resp_header(
...
generate_etag(Req, State=#state{etag=undefined}) ->
	case unsafe_call(Req, State, generate_etag) of
		...
		{Etag, Req2, HandlerState} when is_binary(Etag) ->
			[Etag2] = cowboy1_http:entity_tag_match(Etag),
...
respond(Req, State, StatusCode) ->
	{ok, Req2} = cowboy1_req:reply(StatusCode, Req),
...
rest_terminate(Req, #state{handler=Handler, handler_state=HandlerState}) ->
	case erlang:function_exported(Handler, rest_terminate, 2) of
		true -> ok = Handler:rest_terminate(
			cowboy1_req:lock(Req), HandlerState);
		false -> ok
	end.
