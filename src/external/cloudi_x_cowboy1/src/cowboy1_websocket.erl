...
upgrade(Req, Env, Handler, HandlerOpts) ->
	...
	[Socket, Transport] = cowboy1_req:get([socket, transport], Req),
	...
	try websocket_upgrade(State, Req) of
		...
		after 0 ->
			_ = cowboy1_req:reply(400, Req),
			exit(normal)
		end
	end.

-spec websocket_upgrade(#state{}, Req)
	-> {ok, #state{}, Req} when Req::cowboy1_req:req().
websocket_upgrade(State, Req) ->
	{ok, ConnTokens, Req2}
		= cowboy1_req:parse_header(<<"connection">>, Req),
	true = lists:member(<<"upgrade">>, ConnTokens),
	%% @todo Should probably send a 426 if the Upgrade header is missing.
	{ok, [<<"websocket">>], Req3}
		= cowboy1_req:parse_header(<<"upgrade">>, Req2),
	{Version, Req4} = cowboy1_req:header(<<"sec-websocket-version">>, Req3),
	IntVersion = list_to_integer(binary_to_list(Version)),
	true = (IntVersion =:= 7) orelse (IntVersion =:= 8)
		orelse (IntVersion =:= 13),
	{Key, Req5} = cowboy1_req:header(<<"sec-websocket-key">>, Req4),
	false = Key =:= undefined,
	websocket_extensions(State#state{key=Key},
		cowboy1_req:set_meta(websocket_version, IntVersion, Req5)).

-spec websocket_extensions(#state{}, Req)
	-> {ok, #state{}, Req} when Req::cowboy1_req:req().
websocket_extensions(State, Req) ->
	case cowboy1_req:parse_header(<<"sec-websocket-extensions">>, Req) of
		{ok, Extensions, Req2} when Extensions =/= undefined ->
			[Compress] = cowboy1_req:get([resp_compress], Req),
			case lists:keyfind(<<"x-webkit-deflate-frame">>, 1, Extensions) of
				{<<"x-webkit-deflate-frame">>, []} when Compress =:= true ->
					...
					}, cowboy1_req:set_meta(websocket_compress, true, Req2)};
				_ ->
					{ok, State, cowboy1_req:set_meta(websocket_compress, false, Req2)}
			end;
		_ ->
			{ok, State, cowboy1_req:set_meta(websocket_compress, false, Req)}
	end.

-spec handler_init(#state{}, Req, any())
	-> {ok, Req, cowboy1_middleware:env()} | {suspend, module(), atom(), [any()]}
	when Req::cowboy1_req:req().
...
websocket_handshake(State=#state{
	...
	{ok, Req2} = cowboy1_req:upgrade_reply(
		101,
		[{<<"upgrade">>, <<"websocket">>},
