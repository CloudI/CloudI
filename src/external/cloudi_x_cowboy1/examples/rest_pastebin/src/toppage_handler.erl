...
resource_exists(Req, _State) ->
	case cowboy1_req:binding(paste_id, Req) of
		{undefined, Req2} ->
			{true, Req2, index};
		{PasteID, Req2} ->
...
create_paste(Req, State) ->
	...
	{ok, [{<<"paste">>, Paste}], Req3} = cowboy1_req:body_qs(Req),
	ok = file:write_file(full_path(PasteID), Paste),
	case cowboy1_req:method(Req3) of
		{<<"POST">>, Req4} ->
...
paste_html(Req, Paste) ->
	{Style, Req2} = cowboy1_req:qs_val(<<"lang">>, Req, plain),
