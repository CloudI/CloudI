...
init({_Transport, http}, Req, _Opts) ->
    {Case, Req1} = cowboy1_req:qs_val(<<"case">>, Req),
    case_init(Case, Req1).

case_init(<<"init_before_reply">> = Case, _Req) ->
	cowboy1_error_h:ignore(?MODULE, case_init, 2),
    erlang:error(Case);
case_init(<<"init_after_reply">> = Case, Req) ->
	cowboy1_error_h:ignore(?MODULE, case_init, 2),
    {ok, _Req1} = cowboy1_req:reply(200, [], "http_handler_crashes", Req),
    erlang:error(Case);
case_init(<<"init_reply_handle_error">> = Case, Req) ->
    {ok, Req1} = cowboy1_req:reply(200, [], "http_handler_crashes", Req),
    {ok, Req1, Case};
...
handle(_Req, <<"init_reply_handle_error">> = Case) ->
	cowboy1_error_h:ignore(?MODULE, handle, 2),
    erlang:error(Case);
