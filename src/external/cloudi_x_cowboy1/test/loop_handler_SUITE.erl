...
-import(cowboy1_test, [doc/1]).
-import(cowboy1_test, [gun_open/1]).

%% ct.

all() ->
	cowboy1_test:common_all().

groups() ->
	cowboy1_test:common_groups(cowboy1_test:all(?MODULE)).

init_per_group(Name, Config) ->
	cowboy1_test:init_common_groups(Name, Config, ?MODULE).
