...
to_graphemes(CD0) ->
    case unicode_util_compat:gc(CD0) of
...
-spec trim(String) -> unicode:chardata() when
      String :: unicode:chardata().
trim(Str) ->
    trim(Str, both, unicode_util_compat:whitespace()).

-spec trim(String, Dir) -> unicode:chardata() when
      String :: unicode:chardata(),
      Dir :: direction() | 'both'.
trim(Str, Dir) ->
    trim(Str, Dir, unicode_util_compat:whitespace()).
...
titlecase(CD) when is_list(CD) ->
    case unicode_util_compat:titlecase(CD) of
...
next_grapheme(CD) -> unicode_util_compat:gc(CD).
...
-spec next_codepoint(String::unicode:chardata()) ->
                            ...
                            {error,unicode:chardata()}.
next_codepoint(CD) -> unicode_util_compat:cp(CD).
...
length_1([CP1|[CP2|_]=Cont], N) when ?ASCII_LIST(CP1,CP2) ->
    length_1(Cont, N+1);
length_1(Str, N) ->
    case unicode_util_compat:gc(Str) of
        [] -> N;
...
length_b(Bin0, CP1, N) ->
    [_|Bin1] = unicode_util_compat:gc([CP1|Bin0]),
    case unicode_util_compat:cp(Bin1) of
        [] -> N+1;
...
equal_1(A0,B0) ->
    case {unicode_util_compat:cp(A0), unicode_util_compat:cp(B0)} of
...
equal_nocase(A0, B0) ->
    case {unicode_util_compat:cp(unicode_util_compat:casefold(A0)),
          unicode_util_compat:cp(unicode_util_compat:casefold(B0))} of
...
equal_norm(A0, B0, Norm) ->
    case {unicode_util_compat:cp(unicode_util_compat:Norm(A0)),
          unicode_util_compat:cp(unicode_util_compat:Norm(B0))} of
...
equal_norm_nocase(A0, B0, Norm) ->
    case {unicode_util_compat:cp(unicode_util_compat:casefold(unicode_util_compat:Norm(A0))),
          unicode_util_compat:cp(unicode_util_compat:casefold(unicode_util_compat:Norm(B0)))} of
...
reverse_1(CD, Acc) ->
    case unicode_util_compat:gc(CD) of
...
reverse_b(Bin0, CP1, Acc) ->
    [GC|Bin1] = unicode_util_compat:gc([CP1|Bin0]),
    case unicode_util_compat:cp(Bin1) of
        [] -> [GC|Acc];
...
slice_lb(Bin, CP1, N) ->
    [_|Rest] = unicode_util_compat:gc([CP1|Bin]),
    if N > 1 ->
            case unicode_util_compat:cp(Rest) of
...
slice_bin(CD, CP1, N) when N > 0 ->
    [_|Bin] = unicode_util_compat:gc([CP1|CD]),
    case unicode_util_compat:cp(Bin) of
...
uppercase_list(CPs0, Changed) ->
    case unicode_util_compat:uppercase(CPs0) of
...
uppercase_bin(CP1, Bin, Changed) ->
    case unicode_util_compat:uppercase([CP1|Bin]) of
        [CP1|CPs] ->
...
casefold_list(CPs0, Changed) ->
    case unicode_util_compat:casefold(CPs0) of
...
casefold_bin(CP1, Bin, Changed) ->
    case unicode_util_compat:casefold([CP1|Bin]) of
        [CP1|CPs] ->
            case unicode_util_compat:cp(CPs) of
                [Next|Rest] ->
...
trim_t([CP1|Cont]=Cs0, _, {GCs,CPs,_}=Seps) when is_integer(CP1) ->
    case lists:member(CP1, CPs) of
        true ->
            [GC|Cs1] = unicode_util_compat:gc(Cs0),
...
take_lc([CP1|Cont]=Str0, {GCs,CPs,_}=Seps, Acc) when is_integer(CP1) ->
    case lists:member(CP1, CPs) of
        true ->
            [GC|Str] = unicode_util_compat:gc(Str0),
            case lists:member(GC, GCs) of
...
prefix_1(Cs0, [GC]) ->
    case unicode_util_compat:gc(Cs0) of
        [GC|Cs] -> Cs;
        _ -> nomatch
    end;
...
prefix_1(Cs0, [Pre|PreR]) ->
    case unicode_util_compat:cp(Cs0) of
...
lexemes_m([CP|_]=Cs0, {GCs,CPs,_}=Seps0, Ts) when is_integer(CP) ->
    case lists:member(CP, CPs) of
        true ->
            [GC|Cs2] = unicode_util_compat:gc(Cs0),
            case lists:member(GC, GCs) of
                true ->
...
lexeme_skip([Bin|Cont0], Seps0) when is_binary(Bin) ->
    ...
    case bin_search(Bin, Cont0, Seps) of
        ...
        Cs -> tl(unicode_util_compat:gc(Cs))
    end;
...
lexeme_skip(Bin, Seps0) when is_binary(Bin) ->
    ...
    case bin_search(Bin, [], Seps) of
        {nomatch,_} -> <<>>;
        [Left] -> tl(unicode_util_compat:gc(Left))
    end.
...
find_r([Bin|Cont0], Needle, Res) when is_binary(Bin) ->
    case bin_search_str(Bin, 0, Cont0, Needle) of
        ...
        {_, Cs0, _} ->
            [_|Cs] = unicode_util_compat:gc(Cs0),
...
cp_prefix(Orig, Cont) ->
    case unicode_util_compat:cp(Cont) of
        [] -> Orig;
        [Cp|Rest] -> cp_prefix_1(Orig, Cp, Rest)
    end.

cp_prefix_1(Orig, Until, Cont) ->
    case unicode_util_compat:cp(Orig) of
        [Until|Rest] ->
...
search_cp([Pattern|Seps]) ->
    [CP|_] = unicode_util_compat:cp(Pattern),
    [CP|search_cp(Seps)];
search_cp([]) -> [].
...
bin_search_loop(Bin0, Start, BinSeps, Cont, Seps) ->
    ...
    case binary:match(Bin, BinSeps) of
        ...
        {Where, _CL} when Cont =:= [] ->
            ...
            [GC|Cont2] = unicode_util_compat:gc(Cont1),
...
bin_search_inv_1(<<CP1/utf8, BinRest/binary>>=Bin0, Cont, Sep) ->
    case BinRest of
        ...
        _ when Cont =:= [] ->
            case unicode_util_compat:gc(Bin0) of
                [Sep|Bin] -> bin_search_inv_1(Bin, Cont, Sep);
                _ -> [Bin0|Cont]
            end;
        _ ->
            case unicode_util_compat:gc([Bin0|Cont]) of
...
bin_search_inv_n(<<CP1/utf8, BinRest/binary>>=Bin0, Cont, Seps) ->
    case BinRest of
        ...
        _ when Cont =:= [] ->
            [GC|Bin] = unicode_util_compat:gc(Bin0),
            case lists:member(GC, Seps) of
                true -> bin_search_inv_n(Bin, Cont, Seps);
                false -> [Bin0|Cont]
            end;
        _ ->
            [GC|Cs0] = unicode_util_compat:gc([Bin0|Cont]),
            case lists:member(GC, Seps) of
                false -> [Bin0|Cont];
                true ->
                    case Cs0 of
...
bin_search_str_2(Bin0, Start, Cont, First, SearchCPs) ->
    ...
    case binary:match(Bin, First) of
        ...
        {Where0, _} ->
            Where = Start+Where0,
            <<Keep:Where/binary, Cs0/binary>> = Bin0,
            [GC|Cs]=unicode_util_compat:gc(Cs0),
