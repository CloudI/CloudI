...
-spec method(#http_req{}) -> {cowboy_old_http:method(), #http_req{}}.
method(Req) ->
	{Req#http_req.method, Req}.

%% @doc Return the HTTP version used for the request.
-spec version(#http_req{}) -> {cowboy_old_http:version(), #http_req{}}.
version(Req) ->
...
-spec host(#http_req{}) -> {cowboy_old_dispatcher:tokens(), #http_req{}}.
host(Req) ->
...
-spec host_info(#http_req{})
	-> {cowboy_old_dispatcher:tokens() | undefined, #http_req{}}.
host_info(Req) ->
...
-spec path(#http_req{}) -> {cowboy_old_dispatcher:tokens(), #http_req{}}.
path(Req) ->
...
qs_val(Name, Req=#http_req{raw_qs=RawQs, qs_vals=undefined,
	...
	QsVals = cowboy_old_http:x_www_form_urlencoded(
...
-spec headers(#http_req{}) -> {cowboy_old_http:headers(), #http_req{}}.
headers(Req) ->
...
%% @see parse_header/3
-spec parse_header(cowboy_old_http:header(), #http_req{})
...
-spec parse_header_default(cowboy_old_http:header()) -> any().
...
-spec parse_header(cowboy_old_http:header(), #http_req{}, any())
	-> {any(), #http_req{}} | {undefined, binary(), #http_req{}}
	| {error, badarg}.
parse_header(Name, Req, Default) when Name =:= 'Accept' ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy_old_http:list(Value, fun cowboy_old_http:media_range/2)
		end);
parse_header(Name, Req, Default) when Name =:= 'Accept-Charset' ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy_old_http:nonempty_list(Value, fun cowboy_old_http:conneg/2)
		end);
parse_header(Name, Req, Default) when Name =:= 'Accept-Encoding' ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy_old_http:list(Value, fun cowboy_old_http:conneg/2)
		end);
parse_header(Name, Req, Default) when Name =:= 'Accept-Language' ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy_old_http:nonempty_list(Value, fun cowboy_old_http:language_range/2)
		end);
parse_header(Name, Req, Default) when Name =:= 'Connection' ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy_old_http:nonempty_list(Value, fun cowboy_old_http:token_ci/2)
		end);
parse_header(Name, Req, Default) when Name =:= 'Content-Length' ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy_old_http:digits(Value)
		end);
parse_header(Name, Req, Default) when Name =:= 'Content-Type' ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy_old_http:content_type(Value)
		end);
parse_header(Name, Req, Default) when Name =:= <<"Expect">> ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy_old_http:nonempty_list(Value, fun cowboy_old_http:expectation/2)
		end);
parse_header(Name, Req, Default)
		when Name =:= 'If-Match'; Name =:= 'If-None-Match' ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy_old_http:entity_tag_match(Value)
		end);
parse_header(Name, Req, Default)
		when Name =:= 'If-Modified-Since'; Name =:= 'If-Unmodified-Since' ->
	parse_header(Name, Req, Default,
		fun (Value) ->
			cowboy_old_http:http_date(Value)
		end);
...
cookie(Name, Req=#http_req{cookies=undefined}, Default) when is_binary(Name) ->
	case header('Cookie', Req) of
		...
		{RawCookie, Req2} ->
			Cookies = cowboy_old_cookies:parse_cookie(RawCookie),
...
%%
%% Standard encodings can be found in cowboy_old_http.
...
stream_body(Req=#http_req{body_state=waiting,
	...
	case parse_header(<<"Expect">>, Req) of
		{[<<"100-continue">>], Req1} ->
			HTTPVer = cowboy_old_http:version_to_binary(Version),
	...
	case parse_header('Transfer-Encoding', Req1) of
		{[<<"chunked">>], Req2} ->
			stream_body(Req2#http_req{body_state=
				{stream, fun cowboy_old_http:te_chunked/2, {0, 0},
				 fun cowboy_old_http:ce_identity/1}});
		{[<<"identity">>], Req2} ->
			...
			case Length of
				...
				Length ->
					stream_body(Req3#http_req{body_state=
						{stream, fun cowboy_old_http:te_identity/2, {0, Length},
						 fun cowboy_old_http:ce_identity/1}})
			end
	end;
...
body_qs(Req=#http_req{urldecode={URLDecFun, URLDecArg}}) ->
	...
	{cowboy_old_http:x_www_form_urlencoded(
...
-spec multipart_data(#http_req{})
		-> {{headers, cowboy_old_http:headers()}
...
multipart_data(Req=#http_req{body_state=waiting}) ->
	...
	multipart_data(Req3, Length, {more, cowboy_old_multipart:parser(Boundary)});
...
-spec set_resp_cookie(binary(), binary(), [cowboy_old_cookies:cookie_option()],
	#http_req{}) -> {ok, #http_req{}}.
set_resp_cookie(Name, Value, Options, Req) ->
	{HeaderName, HeaderValue} = cowboy_old_cookies:cookie(Name, Value, Options),
	set_resp_header(HeaderName, HeaderValue, Req).

%% @doc Add a header to the response.
-spec set_resp_header(cowboy_old_http:header(), iodata(), #http_req{})
	-> {ok, #http_req{}}.
...
%% `reply/3'.
%%
%% @see cowboy_old_http_req:transport/1.
...
-spec has_resp_header(cowboy_old_http:header(), #http_req{}) -> boolean().
...
-spec reply(cowboy_old_http:status(), #http_req{}) -> {ok, #http_req{}}.
reply(Status, Req=#http_req{resp_body=Body}) ->
	reply(Status, [], Body, Req).

%% @equiv reply(Status, Headers, [], Req)
-spec reply(cowboy_old_http:status(), cowboy_old_http:headers(), #http_req{})
	-> {ok, #http_req{}}.
reply(Status, Headers, Req=#http_req{resp_body=Body}) ->
	reply(Status, Headers, Body, Req).

%% @doc Send a reply to the client.
-spec reply(cowboy_old_http:status(), cowboy_old_http:headers(), iodata(), #http_req{})
	-> {ok, #http_req{}}.
...
-spec chunked_reply(cowboy_old_http:status(), #http_req{}) -> {ok, #http_req{}}.
chunked_reply(Status, Req) ->
	chunked_reply(Status, [], Req).

%% @doc Initiate the sending of a chunked reply to the client.
%% @see cowboy_old_http_req:chunk/2
-spec chunked_reply(cowboy_old_http:status(), cowboy_old_http:headers(), #http_req{})
	-> {ok, #http_req{}}.
...
%% @private
-spec upgrade_reply(cowboy_old_http:status(), cowboy_old_http:headers(), #http_req{})
	-> {ok, #http_req{}}.
...
-spec response(cowboy_old_http:status(), cowboy_old_http:headers(),
	cowboy_old_http:headers(), cowboy_old_http:headers(), #http_req{})
...
-spec response_connection(cowboy_old_http:headers(), keepalive | close)
	-> keepalive | close.
...
response_connection([{Name, Value}|Tail], Connection) ->
	case Name of
		...
		Name ->
			Name2 = cowboy_old_bstr:to_lower(Name),
			case Name2 of
				<<"connection">> -> response_connection_parse(Value);
...
response_connection_parse(ReplyConn) ->
	Tokens = cowboy_old_http:nonempty_list(ReplyConn, fun cowboy_old_http:token/2),
	cowboy_old_http:connection_to_atom(Tokens).

-spec response_merge_headers(cowboy_old_http:headers(), cowboy_old_http:headers(),
	cowboy_old_http:headers()) -> cowboy_old_http:headers().
...
-spec merge_headers(cowboy_old_http:headers(), cowboy_old_http:headers())
	-> cowboy_old_http:headers().
...
-spec status(cowboy_old_http:status()) -> binary().
...
-spec header_to_binary(cowboy_old_http:header()) -> binary().
