...
%% in your <em>cowboy_old_http_handler:init/3</em> handler function.
...
upgrade(_ListenerPid, Handler, Opts, Req) ->
	try
		...
		{ok, _Req2} = cowboy_old_http_req:reply(500, Req),
		close
	end.
...
method_not_allowed(Req, State, Methods) ->
	{ok, Req2} = cowboy_old_http_req:set_resp_header(
...
is_authorized(Req, State) ->
	case call(Req, State, is_authorized) of
		...
		{{false, AuthHead}, Req2, HandlerState} ->
			{ok, Req3} = cowboy_old_http_req:set_resp_header(
...
content_types_provided(Req=#http_req{meta=Meta}, State) ->
	case call(Req, State, content_types_provided) of
		...
		{CTP, Req2, HandlerState} ->
			...
			{Accept, Req3} = cowboy_old_http_req:parse_header('Accept', Req2),
			case Accept of
				undefined ->
...
normalize_content_types({ContentType, Callback})
    ...
    {cowboy_old_http:content_type(ContentType), Callback};
...
languages_provided(Req, State) ->
	case call(Req, State, languages_provided) of
		...
		{LP, Req2, HandlerState} ->
			...
			{AcceptLanguage, Req3} =
				cowboy_old_http_req:parse_header('Accept-Language', Req2),
...
charsets_provided(Req, State) ->
	case call(Req, State, charsets_provided) of
		...
		{CP, Req2, HandlerState} ->
			...
			{AcceptCharset, Req3} =
				cowboy_old_http_req:parse_header('Accept-Charset', Req2),
...
variances(Req, State=#state{content_types_p=CTP,
	...
	case [[<<", ">>, V] || V <- Variances4] of
		...
		[[<<", ">>, H]|Variances5] ->
			{ok, Req4} = cowboy_old_http_req:set_resp_header(
...
if_match_exists(Req, State) ->
	case cowboy_old_http_req:parse_header('If-Match', Req) of
		{undefined, Req2} ->
...
if_match_musnt_exist(Req, State) ->
	case cowboy_old_http_req:header('If-Match', Req) of
...
if_unmodified_since_exists(Req, State) ->
	case cowboy_old_http_req:parse_header('If-Unmodified-Since', Req) of
		{undefined, Req2} ->
...
if_none_match_exists(Req, State) ->
	case cowboy_old_http_req:parse_header('If-None-Match', Req) of
		{undefined, Req2} ->
...
if_modified_since_exists(Req, State) ->
	case cowboy_old_http_req:parse_header('If-Modified-Since', Req) of
		{undefined, Req2} ->
			method(Req2, State);
		{{error, badarg}, Req2} ->
			method(Req2, State);
...
%%     {PutPath, Req2} = cowboy_old_http_req:meta(put_path, Req)
put_resource(Req, State, OnTrue) ->
	case call(Req, State, content_types_accepted) of
		...
		{CTA, Req2, HandlerState} ->
			...
			State2 = State#state{handler_state=HandlerState},
			{ContentType, Req3}
				= cowboy_old_http_req:parse_header('Content-Type', Req2),
...
is_new_resource(Req, State) ->
	case cowboy_old_http_req:has_resp_header(<<"Location">>, Req) of
		true -> respond(Req, State, 201);
		false -> has_resp_body(Req, State)
	end.

has_resp_body(Req, State) ->
	case cowboy_old_http_req:has_resp_body(Req) of
	...
	case call(Req5, State4, Fun) of
		...
		{Body, Req6, HandlerState} ->
			...
			{ok, Req7} = case Body of
				{stream, Len, Fun1} ->
					cowboy_old_http_req:set_resp_body_fun(Len, Fun1, Req6);
				_Contents ->
					cowboy_old_http_req:set_resp_body(Body, Req6)
			end,
...
generate_etag(Req, State=#state{etag=undefined}) ->
	case call(Req, State, generate_etag) of
		...
		{Etag, Req2, HandlerState} when is_binary(Etag) ->
			[Etag2] = cowboy_old_http:entity_tag_match(Etag),
...
respond(Req, State, StatusCode) ->
	{ok, Req2} = cowboy_old_http_req:reply(StatusCode, Req),
