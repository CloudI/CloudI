...
-module(cowboy_old_http_websocket).
...
upgrade(ListenerPid, Handler, Opts, Req) ->
	cowboy_old_listener:move_connection(ListenerPid, websocket, self()),
...
websocket_upgrade(State, Req) ->
	{ConnTokens, Req2}
		= cowboy_old_http_req:parse_header('Connection', Req),
	true = lists:member(<<"upgrade">>, ConnTokens),
	%% @todo Should probably send a 426 if the Upgrade header is missing.
	{[<<"websocket">>], Req3} = cowboy_old_http_req:parse_header('Upgrade', Req2),
	{Version, Req4} = cowboy_old_http_req:header(<<"Sec-Websocket-Version">>, Req3),
...
websocket_upgrade(undefined, State, Req=#http_req{meta=Meta}) ->
	{Origin, Req2} = cowboy_old_http_req:header(<<"Origin">>, Req),
	{Key1, Req3} = cowboy_old_http_req:header(<<"Sec-Websocket-Key1">>, Req2),
	{Key2, Req4} = cowboy_old_http_req:header(<<"Sec-Websocket-Key2">>, Req3),
...
websocket_upgrade(Version, State, Req=#http_req{meta=Meta})
		when Version =:= <<"7">>; Version =:= <<"8">>;
			Version =:= <<"13">> ->
	{Key, Req2} = cowboy_old_http_req:header(<<"Sec-Websocket-Key">>, Req),
	false = Key =:= undefined,
...
upgrade_error(Req) ->
	{ok, _Req2} = cowboy_old_http_req:reply(400, [], [],
		Req#http_req{resp_state=waiting}),
	closed.

%% @see cowboy_old_http_protocol:ensure_response/1
-spec upgrade_denied(#http_req{}) -> closed.
upgrade_denied(#http_req{resp_state=done}) ->
	closed;
upgrade_denied(Req=#http_req{resp_state=waiting}) ->
	{ok, _Req2} = cowboy_old_http_req:reply(400, [], [], Req),
	closed;
...
websocket_handshake(State=#state{version=0, origin=Origin,
	...
	{ok, Req2} = cowboy_old_http_req:upgrade_reply(
	...
	{ok, Req3} = cowboy_old_http_req:init_stream(
		fun cowboy_old_http:te_identity/2, {0, 8},
		fun cowboy_old_http:ce_identity/1, Req2),
	case cowboy_old_http_req:body(Req3) of
		{ok, Key3, Req4} ->
