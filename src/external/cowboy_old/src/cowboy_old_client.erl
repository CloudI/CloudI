...
-record(client, {
	...
	version = {1, 1} :: cowboy_old_http:version(),
...
request(Method, URL, Headers, Body, Client=#client{
	...
	VersionBin = cowboy_old_http:version_to_binary(Version),
...
parse_url(<< "https://", Rest/binary >>) ->
	parse_url(Rest, cowboy_old_ssl_transport);
parse_url(<< "http://", Rest/binary >>) ->
	parse_url(Rest, cowboy_old_tcp_transport);
parse_url(URL) ->
	parse_url(URL, cowboy_old_tcp_transport).
...
parse_peer(Peer, Transport) ->
	case binary:split(Peer, <<":">>) of
		[Host] when Transport =:= cowboy_old_tcp_transport ->
			{binary_to_list(Host), 80};
		[Host] when Transport =:= cowboy_old_ssl_transport ->
