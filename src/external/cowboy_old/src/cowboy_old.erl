...
%% to 1024. See <em>cowboy_old_listener</em> for more details on limiting the number
%% of connections.
%%
%% Although Cowboy includes a <em>cowboy_old_http_protocol</em> handler, other
...
start_listener(Ref, NbAcceptors, Transport, TransOpts, Protocol, ProtoOpts)
	...
	supervisor:start_child(cowboy_old_sup, child_spec(Ref, NbAcceptors,
		Transport, TransOpts, Protocol, ProtoOpts)).

%% @doc Stop a listener identified by <em>Ref</em>.
-spec stop_listener(any()) -> ok | {error, not_found}.
stop_listener(Ref) ->
	case supervisor:terminate_child(cowboy_old_sup, {cowboy_old_listener_sup, Ref}) of
		ok ->
			supervisor:delete_child(cowboy_old_sup, {cowboy_old_listener_sup, Ref});
		{error, Reason} ->
			{error, Reason}
	end.
...
%%
%% When you want to embed cowboy_old in another application, you can use this
...
%% than hooking the listener to the cowboy_old internal supervisor, it just returns
%% the spec.
...
child_spec(Ref, NbAcceptors, Transport, TransOpts, Protocol, ProtoOpts)
		when is_integer(NbAcceptors) andalso is_atom(Transport)
		andalso is_atom(Protocol) ->
	{{cowboy_old_listener_sup, Ref}, {cowboy_old_listener_sup, start_link, [
		NbAcceptors, Transport, TransOpts, Protocol, ProtoOpts
	]}, permanent, 5000, supervisor, [cowboy_old_listener_sup]}.
...
get_protocol_options(Ref) ->
	...
	{ok, ProtoOpts} = cowboy_old_listener:get_protocol_options(ListenerPid),
	ProtoOpts.
...
set_protocol_options(Ref, ProtoOpts) ->
	ListenerPid = ref_to_listener_pid(Ref),
	ok = cowboy_old_listener:set_protocol_options(ListenerPid, ProtoOpts).

%% Internal.

-spec ref_to_listener_pid(any()) -> pid().
ref_to_listener_pid(Ref) ->
	Children = supervisor:which_children(cowboy_old_sup),
	{_, ListenerSupPid, _, _} = lists:keyfind(
		{cowboy_old_listener_sup, Ref}, 1, Children),
	ListenerSupChildren = supervisor:which_children(ListenerSupPid),
	{_, ListenerPid, _, _} = lists:keyfind(
		cowboy_old_listener, 1, ListenerSupChildren),
	ListenerPid.
