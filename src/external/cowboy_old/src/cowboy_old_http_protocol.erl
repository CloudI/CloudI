...
%%   URL encoded strings. Defaults to `{fun cowboy_old_http:urldecode/2, crash}'.
%%   </dd>
%% </dl>
%%
...
%%
%% @see cowboy_old_dispatcher
%% @see cowboy_old_http_handler
-module(cowboy_old_http_protocol).
-behaviour(cowboy_old_protocol).
...
-record(state, {
	...
	socket :: inet:socket(),
	transport :: module(),
	dispatch :: cowboy_old_dispatcher:dispatch_rules(),
	handler :: {module(), any()},
	onrequest :: undefined | fun((#http_req{}) -> #http_req{}),
	onresponse = undefined :: undefined | fun((cowboy_old_http:status(),
		cowboy_old_http:headers(), #http_req{}) -> #http_req{}),
...
init(ListenerPid, Socket, Transport, Opts) ->
	...
	URLDecDefault = {fun cowboy_old_http:urldecode/2, crash},
	URLDec = proplists:get_value(urldecode, Opts, URLDecDefault),
	ok = cowboy_old:accept_ack(ListenerPid),
...
-spec request({http_request, cowboy_old_http:method(), cowboy_old_http:uri(),
	cowboy_old_http:version()}, #state{}) -> ok.
...
request({http_request, Method, {abs_path, AbsPath}, Version},
	...
	{Path, RawPath, Qs} = cowboy_old_dispatcher:split_path(AbsPath, URLDecode),
...
-spec header({http_header, integer(), cowboy_old_http:header(), any(), binary()}
	| http_eoh, #http_req{}, #state{}) -> ok.
header({http_header, _I, 'Host', _R, RawHost}, Req=#http_req{
		transport=Transport, host=undefined}, State) ->
	RawHost2 = cowboy_old_bstr:to_lower(RawHost),
	case catch cowboy_old_dispatcher:split_host(RawHost2) of
...
header({http_header, _I, 'Connection', _R, Connection},
	...
	{ConnTokens, Req3}
		= cowboy_old_http_req:parse_header('Connection', Req2),
	ConnAtom = cowboy_old_http:connection_to_atom(ConnTokens),
...
dispatch(Req=#http_req{host=Host, path=Path},
	...
	case cowboy_old_dispatcher:match(Host, Path, Dispatch) of
...
%% Almost the same code can be found in cowboy_old_http_websocket.
...
next_request(Req=#http_req{connection=Conn}, State=#state{
	...
	receive {cowboy_old_http_req, resp_sent} -> ok after 0 -> ok end,
...
ensure_response(Req=#http_req{resp_state=waiting}) ->
	_ = cowboy_old_http_req:reply(204, [], [], Req),
	ok;
...
-spec error_terminate(cowboy_old_http:status(), #state{}) -> ok.
error_terminate(Code, State=#state{socket=Socket, transport=Transport,
		onresponse=OnResponse}) ->
	receive
		{cowboy_old_http_req, resp_sent} -> ok
	after 0 ->
		_ = cowboy_old_http_req:reply(Code, #http_req{
...
-spec version_to_connection(cowboy_old_http:version()) -> keepalive | close.
...
format_header(<< C, Rest/bits >>, true, Acc) ->
	format_header(Rest, false, << Acc/binary, (cowboy_old_bstr:char_to_upper(C)) >>);
format_header(<< C, Rest/bits >>, false, Acc) ->
	format_header(Rest, false, << Acc/binary, (cowboy_old_bstr:char_to_lower(C)) >>).
