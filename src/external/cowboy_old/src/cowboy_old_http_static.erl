...
%% The mimetypes must be of a type that the `cowboy_old_http_rest' protocol can
%% handle.
%%
...
%% {['*', <<"page">>], cowboy_old_http_static,
%%     [{directory, {priv_dir, cowboy, [<<"www">>]}}
%%      {file, <<"page.html">>}]}.
%%
%% %% Always serve cowboy/priv/www/other.html under http://example.com/other
%% {[<<"other">>, '...'], cowboy_old_http_static,
%%     [{directory, {priv_dir, cowboy, [<<"www">>]}}
%%      {file, "other.html"}]}
%% '''
-module(cowboy_old_http_static).

%% include files
-include("http.hrl").
-include_lib("kernel/include/file.hrl").

%% cowboy_old_http_protocol callbacks
-export([init/3]).

%% cowboy_old_http_rest callbacks
...
init({_Transport, http}, _Req, _Opts) ->
	{upgrade, protocol, cowboy_old_http_rest}.
...
rest_init(Req, Opts) ->
	...
	{Filepath, Req1} = case lists:keyfind(file, 1, Opts) of
		...
		false -> cowboy_old_http_req:path_info(Req)
	end,
...
file_contents(Req, #state{filepath=Filepath,
	...
	{ok, Transport, Socket} = cowboy_old_http_req:transport(Req),
...
content_function(Transport, Socket, Filepath) ->
	%% `file:sendfile/2' will only work with the `cowboy_old_tcp_transport'
	...
	case erlang:function_exported(file, sendfile, 2) of
		...
		_ when Transport =/= cowboy_old_tcp_transport ->
			fun() -> sfallback(Transport, Socket, Filepath) end;
		true ->
