...
%% @private
-module(cowboy_old_listener_sup).
...
start_link(NbAcceptors, Transport, TransOpts, Protocol, ProtoOpts) ->
	...
	{ok, ListenerPid} = supervisor:start_child(SupPid,
		{cowboy_old_listener, {cowboy_old_listener, start_link, [MaxConns, ProtoOpts]},
		 permanent, 5000, worker, [cowboy_old_listener]}),
	{ok, ReqsPid} = supervisor:start_child(SupPid,
		{cowboy_old_requests_sup, {cowboy_old_requests_sup, start_link, []},
		 permanent, 5000, supervisor, [cowboy_old_requests_sup]}),
	{ok, _PoolPid} = supervisor:start_child(SupPid,
		{cowboy_old_acceptors_sup, {cowboy_old_acceptors_sup, start_link, [
			NbAcceptors, Transport, TransOpts,
			Protocol, ProtoOpts, ListenerPid, ReqsPid
		]}, permanent, 5000, supervisor, [cowboy_old_acceptors_sup]}),
	{ok, SupPid}.
