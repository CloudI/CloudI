...
brutal_kill(_) ->
	...
	Name = name(),
	{ok, ListenerSup} = ranch1:start_listener(Name, 1,
		ranch1_tcp, [{port, 0}, {shutdown, brutal_kill}],
		echo_protocol, []),
	Port = ranch1:get_port(Name),
	{ok, _} = gen_tcp:connect("localhost", Port, []),
	receive after 100 -> ok end,
	ListenerSupChildren = supervisor:which_children(ListenerSup),
	{_, ConnsSup, _, _} = lists:keyfind(ranch1_conns_sup, 1, ListenerSupChildren),
	[{_, Pid, _, _}] = supervisor:which_children(ConnsSup),
	true = is_process_alive(Pid),
	ok = ranch1:stop_listener(Name),
	receive after 100 -> ok end,
...
infinity(_) ->
	...
	Name = name(),
	{ok, ListenerSup} = ranch1:start_listener(Name, 1,
		ranch1_tcp, [{port, 0}, {shutdown, infinity}],
		echo_protocol, []),
	Port = ranch1:get_port(Name),
	{ok, _} = gen_tcp:connect("localhost", Port, []),
	receive after 100 -> ok end,
...
infinity_trap_exit(_) ->
	...
	SpawnPid = spawn(fun() -> ok = ranch1:stop_listener(Name) end),
	receive after 100 -> ok end,
...
timeout(_) ->
	...
	Name = name(),
	{ok, ListenerSup} = ranch1:start_listener(Name, 1,
		ranch1_tcp, [{port, 0}, {shutdown, 500}],
		echo_protocol, []),
	Port = ranch1:get_port(Name),
	{ok, _} = gen_tcp:connect("localhost", Port, []),
	receive after 100 -> ok end,
