...
start_listener(Ref, NumAcceptors, Transport, TransOpts, Protocol, ProtoOpts)
	...
	case erlang:function_exported(Transport, name, 0) of
		false ->
			{error, badarg};
		true ->
			Res = supervisor:start_child(ranch1_sup, child_spec(Ref, NumAcceptors,
					Transport, TransOpts, Protocol, ProtoOpts)),
			Socket = proplists:get_value(socket, TransOpts),
			case Res of
				{ok, Pid} when Socket =/= undefined ->
					%% Give ownership of the socket to ranch1_acceptors_sup
					...
					{_, AcceptorsSup, _, _}
						= lists:keyfind(ranch1_acceptors_sup, 1, Children),
...
maybe_started({error, {{shutdown,
		{failed_to_start_child, ranch1_acceptors_sup,
...
stop_listener(Ref) ->
	case supervisor:terminate_child(ranch1_sup, {ranch1_listener_sup, Ref}) of
		ok ->
			_ = supervisor:delete_child(ranch1_sup, {ranch1_listener_sup, Ref}),
			ranch1_server:cleanup_listener_opts(Ref);
		{error, Reason} ->
			{error, Reason}
	end.
...
child_spec(Ref, NumAcceptors, Transport, TransOpts, Protocol, ProtoOpts)
		when is_integer(NumAcceptors) andalso is_atom(Transport)
		andalso is_atom(Protocol) ->
	{{ranch1_listener_sup, Ref}, {ranch1_listener_sup, start_link, [
		Ref, NumAcceptors, Transport, TransOpts, Protocol, ProtoOpts
	]}, permanent, infinity, supervisor, [ranch1_listener_sup]}.
...
get_max_connections(Ref) ->
	ranch1_server:get_max_connections(Ref).

-spec set_max_connections(ref(), max_conns()) -> ok.
set_max_connections(Ref, MaxConnections) ->
	ranch1_server:set_max_connections(Ref, MaxConnections).

-spec get_protocol_options(ref()) -> any().
get_protocol_options(Ref) ->
	ranch1_server:get_protocol_options(Ref).

-spec set_protocol_options(ref(), any()) -> ok.
set_protocol_options(Ref, Opts) ->
	ranch1_server:set_protocol_options(Ref, Opts).

-spec info() -> [{any(), [{atom(), any()}]}].
info() ->
	Children = supervisor:which_children(ranch1_sup),
	[{Ref, listener_info(Ref, Pid)}
		|| {{ranch1_listener_sup, Ref}, Pid, _, [_]} <- Children].
...
listener_info(Ref, Pid) ->
	...
	[
		...
		{active_connections, ranch1_conns_sup:active_connections(ConnsSup)},
...
listener_start_args(Ref) ->
	case erlang:function_exported(supervisor, get_childspec, 2) of
		true ->
			...
			{ok, Map} = supervisor:get_childspec(ranch1_sup, {ranch1_listener_sup, Ref}),
			{ranch1_listener_sup, start_link, StartArgs} = maps:get(start, Map),
			StartArgs;
		false ->
			%% Awful solution for compatibility with R16 and R17.
			{status, _, _, [_, _, _, _, [_, _,
				{data, [{_, {state, _, _, Children, _, _, _, _, _, _}}]}]]}
				= sys:get_status(ranch1_sup),
			[StartArgs] = [StartArgs || {child, _, {ranch1_listener_sup, ChildRef},
				{ranch1_listener_sup, start_link, StartArgs}, _, _, _, _}
...
procs(Ref, acceptors) ->
	procs1(Ref, ranch1_acceptors_sup);
procs(Ref, connections) ->
	procs1(Ref, ranch1_conns_sup).

procs1(Ref, Sup) ->
	{_, ListenerSup, _, _} = lists:keyfind({ranch1_listener_sup, Ref}, 1,
		supervisor:which_children(ranch1_sup)),
