...
%% as calling l(ranch1_conns_sup) twice will kill the process and all
%% the currently open connections.
-module(ranch1_conns_sup).
...
-spec init(pid(), ranch1:ref(), conn_type(), shutdown(),
	module(), timeout(), module()) -> no_return().
init(Parent, Ref, ConnType, Shutdown, Transport, AckTimeout, Protocol) ->
	process_flag(trap_exit, true),
	ok = ranch1_server:set_connections_sup(Ref, self()),
	MaxConns = ranch1_server:get_max_connections(Ref),
	Opts = ranch1_server:get_protocol_options(Ref),
