...
-module(ranch1_acceptors_sup).
-behaviour(supervisor).

-export([start_link/4]).
-export([init/1]).

-spec start_link(ranch1:ref(), non_neg_integer(), module(), any())
	-> {ok, pid()}.
...
init([Ref, NumAcceptors, Transport, TransOpts]) ->
	...
	ranch1_server:set_addr(Ref, Addr),
	Procs = [
		{{acceptor, self(), N}, {ranch1_acceptor, start_link, [
