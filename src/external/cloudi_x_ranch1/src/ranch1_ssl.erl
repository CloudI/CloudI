...
-type opt() :: ranch1_tcp:opt() | ssl_opt().
-export_type([opt/0]).
...
do_listen(Opts) ->
	Opts2 = ranch1:set_option_default(Opts, backlog, 1024),
	Opts3 = ranch1:set_option_default(Opts2, ciphers, unbroken_cipher_suites()),
	Opts4 = ranch1:set_option_default(Opts3, nodelay, true),
	Opts5 = ranch1:set_option_default(Opts4, send_timeout, 30000),
	Opts6 = ranch1:set_option_default(Opts5, send_timeout_close, true),
	%% We set the port to 0 because it is given in the Opts directly.
	%% The port in the options takes precedence over the one in the
	%% first argument.
	ssl:listen(0, ranch1:filter_options(Opts6, disallowed_listen_options(),
...
disallowed_listen_options() ->
	[alpn_advertised_protocols, client_preferred_next_protocols,
		...
		|ranch1_tcp:disallowed_listen_options()].
...
-spec sendfile(ssl:sslsocket(), file:name_all() | file:fd(),
		non_neg_integer(), non_neg_integer(), ranch1_transport:sendfile_opts())
	-> {ok, non_neg_integer()} | {error, atom()}.
sendfile(Socket, File, Offset, Bytes, Opts) ->
	ranch1_transport:sendfile(?MODULE, Socket, File, Offset, Bytes, Opts).
