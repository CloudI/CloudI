...
%% UW: Modified copy of file_io_server.erl in OTP 24.3.4.2
%% The modifications (noted with "UW:" comments)
...
%% UW: Instead of Filename, we expect the file contentst as a binary
%%
start(Owner, Data, ModeList) 
  when is_pid(Owner), is_binary(Data), is_list(ModeList) ->
    do_start(spawn, Owner, Data, ModeList).

start_link(Owner, Data, ModeList) 
  when is_pid(Owner), is_binary(Data), is_list(ModeList) ->
    do_start(spawn_link, Owner, Data, ModeList).
	  ...
	  fun() ->
		  ...
		  case parse_options(ModeList) of
                      {ReadMode, UnicodeMode, Opts} ->
                          %% UW: use ram_file instead of raw_file_io
                          %% UW: Don't set read_ahead since ram_file doesn't support it
			  case ram_file:open(FileName, Opts) of
			      {error, Reason} = Error ->
				  Self ! {Ref, Error},
				  exit(Reason);
			      {ok, Handle} ->
...
%% UW: don't add read_ahead, since ram_file doesn't support it
