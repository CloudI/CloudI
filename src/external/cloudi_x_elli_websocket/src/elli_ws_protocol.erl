...
% 	-> {ok, #state{}, Req} when Req::elli_ws_request_adapter:req().
websocket_upgrade(State, Req) ->
	{ok, ConnTokens, Req2} = elli_ws_request_adapter:parse_header(<<"connection">>, Req),
	true = lists:member(<<"upgrade">>, ConnTokens),

	%% @todo Should probably send a 426 if the Upgrade header is missing.
	{ok, [<<"websocket">>], Req3} = elli_ws_request_adapter:parse_header(<<"upgrade">>, Req2),
	{Version, Req4} = elli_ws_request_adapter:header(<<"sec-websocket-version">>, Req3),
	IntVersion = list_to_integer(binary_to_list(Version)),
	true = (IntVersion =:= 7) orelse (IntVersion =:= 8) orelse (IntVersion =:= 13),
	{Key, Req5} = elli_ws_request_adapter:header(<<"sec-websocket-key">>, Req4),
	false = Key =:= undefined,
	websocket_extensions(State#state{key=Key}, elli_ws_request_adapter:set_meta(websocket_version, IntVersion, Req5)).

% -spec websocket_extensions(#state{}, Req)
% 	-> {ok, #state{}, Req} when Req::elli_ws_request_adapter:req().
websocket_extensions(State, Req) ->
	case elli_ws_request_adapter:parse_header(<<"sec-websocket-extensions">>, Req) of
		{ok, Extensions, Req2} when Extensions =/= undefined ->
			[Compress] = elli_ws_request_adapter:get([resp_compress], Req),
			case lists:keyfind(<<"x-webkit-deflate-frame">>, 1, Extensions) of
				{<<"x-webkit-deflate-frame">>, []} when Compress =:= true ->
					...
					}, elli_ws_request_adapter:set_meta(websocket_compress, true, Req2)};
				_ ->
					{ok, State, elli_ws_request_adapter:set_meta(websocket_compress, false, Req2)}
			end;
		_ ->
			{ok, State, elli_ws_request_adapter:set_meta(websocket_compress, false, Req)}
	end.
...
handler_init(State=#state{env=Env, handler=Handler}, Req, HandlerOpts) ->
	try elli_ws_request_adapter:websocket_handler_init(Req, Handler, HandlerOpts) of
...
% 	| {suspend, module(), atom(), [any()]}
% 	when Req::elli_ws_request_adapter:req().
handler_loop(State=#state{socket={_,Port}, messages={OK, Closed, Error},
...
handler_call(State=#state{handler=Handler}, Req, HandlerState,
	...
	try elli_ws_request_adapter:websocket_handler_callback(Req, Handler, Callback, Message, HandlerState) of
...
websocket_send(Type, State=#state{socket=Socket})
		when Type =:= close ->
	Opcode = websocket_opcode(Type),
	case elli_tcp:send(Socket, << 1:1, 0:3, Opcode:4, 0:8 >>) of
		ok -> {shutdown, State};
		Error -> {Error, State}
	end;
websocket_send(Type, State=#state{socket=Socket})
	...
	Opcode = websocket_opcode(Type),
	{elli_tcp:send(Socket, << 1:1, 0:3, Opcode:4, 0:8 >>), State};
