...
init_per_group(_GroupName, Config) ->
    ...
    IndexWithShards = cloudi_x_erlasticsearch:join([Index, <<"with_shards">>], <<"_">>),
...
t_health(Config) ->
    ...
    Response = cloudi_x_erlasticsearch:health(ServerRef),
    true = cloudi_x_erlasticsearch:is_200(Response).
...
t_state(Config) ->
    ServerRef = ?config(pool, Config),
    Response1 = cloudi_x_erlasticsearch:state(ServerRef),
    true = cloudi_x_erlasticsearch:is_200(Response1),
    Response2 = cloudi_x_erlasticsearch:state(ServerRef, [{filter_nodes, true}]),
    true = cloudi_x_erlasticsearch:is_200(Response2).

t_nodes_info(Config) ->
    ServerRef = ?config(pool, Config),
    Response1 = cloudi_x_erlasticsearch:nodes_info(ServerRef),
    true = cloudi_x_erlasticsearch:is_200(Response1).

t_nodes_stats(Config) ->
    ServerRef = ?config(pool, Config),
    Response1 = cloudi_x_erlasticsearch:nodes_stats(ServerRef),
    true = cloudi_x_erlasticsearch:is_200(Response1).
...
check_status_1(ServerRef, Index) ->
    lists:foreach(fun(X) ->
                ...
                Response = cloudi_x_erlasticsearch:status(ServerRef, FullIndex),
                true = cloudi_x_erlasticsearch:is_200(Response)
...
check_indices_stats(ServerRef, Index) ->
    lists:foreach(fun(X) ->
                FullIndex = enumerated(Index, X),
                Response = cloudi_x_erlasticsearch:indices_stats(ServerRef, FullIndex),
                true = cloudi_x_erlasticsearch:is_200(Response)
        end, lists:seq(1, ?DOCUMENT_DEPTH)).
...
check_status_all(ServerRef, Index) ->
    ...
    Response = cloudi_x_erlasticsearch:status(ServerRef, FullIndexList),
    true = cloudi_x_erlasticsearch:is_200(Response).

clear_cache_1(ServerRef, Index) ->
    lists:foreach(fun(X) ->
                FullIndex = enumerated(Index, X),
                Response1 = cloudi_x_erlasticsearch:clear_cache(ServerRef, FullIndex),
                true = cloudi_x_erlasticsearch:is_200(Response1),
                Response2 = cloudi_x_erlasticsearch:clear_cache(ServerRef, FullIndex, [{filter, true}]),
                true = cloudi_x_erlasticsearch:is_200(Response2)
        end, lists:seq(1, ?DOCUMENT_DEPTH)).
...
clear_cache_list(ServerRef, Index) ->
    ...
    Response1 = cloudi_x_erlasticsearch:clear_cache(ServerRef, FullIndexList),
    true = cloudi_x_erlasticsearch:is_200(Response1),
    Response2 = cloudi_x_erlasticsearch:clear_cache(ServerRef, FullIndexList, [{filter, true}]),
    true = cloudi_x_erlasticsearch:is_200(Response2).

clear_cache_all(ServerRef, _Index) ->
    Response1 = cloudi_x_erlasticsearch:clear_cache(ServerRef),
    true = cloudi_x_erlasticsearch:is_200(Response1),
    Response2 = cloudi_x_erlasticsearch:clear_cache(ServerRef, [], [{filter, true}]),
    true = cloudi_x_erlasticsearch:is_200(Response2).
...
are_types_1(ServerRef, Index, Type) ->
    lists:foreach(fun(X) ->
                ...
                lists:foreach(fun(Y) ->
                            ...
                            true = true_response(cloudi_x_erlasticsearch:is_type(ServerRef, FullIndex, FullType))
                    end, lists:seq(1, ?DOCUMENT_DEPTH))
        end, lists:seq(1, ?DOCUMENT_DEPTH)).
...
are_types_all(ServerRef, Index, Type) ->
    ...
    lists:foreach(fun(X) ->
                ...
                true = true_response(cloudi_x_erlasticsearch:is_type(ServerRef, FullIndexList, FullType))
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    % List of types
    lists:foreach(fun(X) ->
                FullIndex = enumerated(Index, X),
                true = true_response(cloudi_x_erlasticsearch:is_type(ServerRef, FullIndex, FullTypeList))
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    % List of indices and types
    true = true_response(cloudi_x_erlasticsearch:is_type(ServerRef, FullIndexList, FullTypeList)).
...
build_data(ServerRef, Index, Type) ->
    lists:foreach(fun(X) ->
                ...
                lists:foreach(fun(Y) ->
                            ...
                            cloudi_x_erlasticsearch:insert_doc(ServerRef, FullIndex, 
                                                      FullType, BX, json_document(X)),
                            cloudi_x_erlasticsearch:flush(ServerRef)
                    end, lists:seq(1, ?DOCUMENT_DEPTH))
        end, lists:seq(1, ?DOCUMENT_DEPTH)).
...
clear_data(ServerRef, Index) ->
    lists:foreach(fun(X) ->
                FullIndex = enumerated(Index, X),
                cloudi_x_erlasticsearch:delete_index(ServerRef, FullIndex)
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    cloudi_x_erlasticsearch:flush(ServerRef).
...
t_put_mapping(Config) ->
    ...
    Index = ?config(index, Config),
    Type = ?config(type, Config),
    cloudi_x_erlasticsearch:create_index(ServerRef, Index),
    MappingDoc = cloudi_x_jsx:encode(?MAPPING_DOC(Type)),
    Response = cloudi_x_erlasticsearch:put_mapping(ServerRef, Index, Type, MappingDoc),
    true = cloudi_x_erlasticsearch:is_200(Response),
...
t_get_mapping(Config) ->
    ...
    cloudi_x_erlasticsearch:create_index(ServerRef, Index),
    MappingDoc = cloudi_x_jsx:encode(?MAPPING_DOC(Type)),
    Response1 = cloudi_x_erlasticsearch:put_mapping(ServerRef, Index, Type, MappingDoc),
    true = cloudi_x_erlasticsearch:is_200(Response1),
    Response2 = cloudi_x_erlasticsearch:get_mapping(ServerRef, Index, Type),
...
t_delete_mapping(Config) ->
    ...
    Response1 = cloudi_x_erlasticsearch:put_mapping(ServerRef, Index, Type, MappingDoc),
    true = cloudi_x_erlasticsearch:is_200(Response1),
    Response2 = cloudi_x_erlasticsearch:delete_mapping(ServerRef, Index, Type),
    true = cloudi_x_erlasticsearch:is_200(Response2),
    delete_this_index(ServerRef, Index).
...
t_aliases(Config) ->
    ...
    Response = cloudi_x_erlasticsearch:aliases(ServerRef, AliasesDoc),
    true = cloudi_x_erlasticsearch:is_200(Response),
    delete_this_index(ServerRef, Index).
...
t_insert_alias_1(Config) ->
    ...
    Alias = random_name(Index),
    cloudi_x_erlasticsearch:create_index(ServerRef, Index),
    Response = cloudi_x_erlasticsearch:insert_alias(ServerRef, Index, Alias),
    true = cloudi_x_erlasticsearch:is_200(Response),
    delete_this_index(ServerRef, Index).
...
t_insert_alias_2(Config) ->
    ...
    cloudi_x_erlasticsearch:create_index(ServerRef, Index),
    Params = [{<<"routing">>, <<"1">>}],
    Response = cloudi_x_erlasticsearch:insert_alias(ServerRef, Index, Alias, Params),
    true = cloudi_x_erlasticsearch:is_200(Response),
    delete_this_index(ServerRef, Index).
...
t_delete_alias(Config) ->
    ...
    Alias = random_name(Index),
    cloudi_x_erlasticsearch:create_index(ServerRef, Index),
    Response1 = cloudi_x_erlasticsearch:insert_alias(ServerRef, Index, Alias),
    true = cloudi_x_erlasticsearch:is_200(Response1),
    Response2 = cloudi_x_erlasticsearch:delete_alias(ServerRef, Index, Alias),
    true = cloudi_x_erlasticsearch:is_200(Response2),
    delete_this_index(ServerRef, Index).
...
t_is_alias(Config) ->
    ...
    Response1 = cloudi_x_erlasticsearch:insert_alias(ServerRef, Index, Alias),
    true = cloudi_x_erlasticsearch:is_200(Response1),
    true = true_response(cloudi_x_erlasticsearch:is_alias(ServerRef, Index, Alias)),
    delete_this_index(ServerRef, Index).
...
t_get_alias(Config) ->
    ...
    Response1 = cloudi_x_erlasticsearch:insert_alias(ServerRef, Index, Alias),
    true = cloudi_x_erlasticsearch:is_200(Response1),
    Response2 = cloudi_x_erlasticsearch:get_alias(ServerRef, Index, Alias),
...
t_flush_1(Config) ->
    ...
    lists:foreach(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                FullIndex = cloudi_x_erlasticsearch:join([Index, BX], <<"_">>),
                Response = cloudi_x_erlasticsearch:flush(ServerRef, FullIndex),
                true = cloudi_x_erlasticsearch:is_200(Response)
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    delete_all_indices(ServerRef, Index).
...
t_flush_list(Config) ->
    ...
    lists:map(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                cloudi_x_erlasticsearch:join([Index, BX], <<"_">>)
            end, lists:seq(1, ?DOCUMENT_DEPTH)),
    Response = cloudi_x_erlasticsearch:flush(ServerRef, Indexes),
    true = cloudi_x_erlasticsearch:is_200(Response),
    delete_all_indices(ServerRef, Index).
...
t_flush_all(Config) ->
    ...
    Index = ?config(index, Config),
    create_indices(ServerRef, Index),
    Response = cloudi_x_erlasticsearch:flush(ServerRef),
    true = cloudi_x_erlasticsearch:is_200(Response),
    delete_all_indices(ServerRef, Index).
...
t_refresh_1(Config) ->
    ...
    lists:foreach(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                FullIndex = cloudi_x_erlasticsearch:join([Index, BX], <<"_">>),
                Response = cloudi_x_erlasticsearch:refresh(ServerRef, FullIndex),
                true = cloudi_x_erlasticsearch:is_200(Response)
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    delete_all_indices(ServerRef, Index).
...
t_refresh_list(Config) ->
    ...
    Response = cloudi_x_erlasticsearch:refresh(ServerRef, Indexes),
    true = cloudi_x_erlasticsearch:is_200(Response),
    delete_all_indices(ServerRef, Index).
...
t_refresh_all(Config) ->
    ...
    Index = ?config(index, Config),
    create_indices(ServerRef, Index),
    Response = cloudi_x_erlasticsearch:refresh(ServerRef),
    true = cloudi_x_erlasticsearch:is_200(Response),
    delete_all_indices(ServerRef, Index).
...
t_optimize_1(Config) ->
    ...
    lists:foreach(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                FullIndex = cloudi_x_erlasticsearch:join([Index, BX], <<"_">>),
                Response = cloudi_x_erlasticsearch:optimize(ServerRef, FullIndex),
                true = cloudi_x_erlasticsearch:is_200(Response)
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    delete_all_indices(ServerRef, Index).
...
t_optimize_list(Config) ->
    ...
    Response = cloudi_x_erlasticsearch:optimize(ServerRef, Indexes),
    true = cloudi_x_erlasticsearch:is_200(Response),
    delete_all_indices(ServerRef, Index).
...
t_optimize_all(Config) ->
    ...
    Index = ?config(index, Config),
    create_indices(ServerRef, Index),
    Response = cloudi_x_erlasticsearch:optimize(ServerRef),
    true = cloudi_x_erlasticsearch:is_200(Response),
    delete_all_indices(ServerRef, Index).
...
t_segments_1(Config) ->
    ...
    lists:foreach(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                FullIndex = cloudi_x_erlasticsearch:join([Index, BX], <<"_">>),
                Response = cloudi_x_erlasticsearch:segments(ServerRef, FullIndex),
                true = cloudi_x_erlasticsearch:is_200(Response)
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    delete_all_indices(ServerRef, Index).
...
t_segments_list(Config) ->
    ...
    Response = cloudi_x_erlasticsearch:segments(ServerRef, Indexes),
    true = cloudi_x_erlasticsearch:is_200(Response),
    delete_all_indices(ServerRef, Index).
...
t_segments_all(Config) ->
    ...
    Index = ?config(index, Config),
    create_indices(ServerRef, Index),
    Response = cloudi_x_erlasticsearch:segments(ServerRef),
    true = cloudi_x_erlasticsearch:is_200(Response),
    delete_all_indices(ServerRef, Index).
...
t_open_index(Config) ->
    ...
    Response = cloudi_x_erlasticsearch:close_index(ServerRef, Index),
    true = cloudi_x_erlasticsearch:is_200(Response),
    Response1 = cloudi_x_erlasticsearch:open_index(ServerRef, Index),
    true = cloudi_x_erlasticsearch:is_200(Response1),
...
t_mget_id(Config) ->
    ...
    Query = id_query(),
    process_t_insert_doc(ServerRef, Config),
    Result = cloudi_x_erlasticsearch:mget_doc(ServerRef, Index, Type, Query),
...
t_mget_type(Config) ->
    ...
    Query = id_query(Type),
    process_t_insert_doc(ServerRef, Config),
    Result = cloudi_x_erlasticsearch:mget_doc(ServerRef, Index, Query),
    ?DOCUMENT_DEPTH  = docs_from_result(Result),
    process_t_delete_doc(ServerRef, Config).
...
t_mget_index(Config) ->
    ...
    process_t_insert_doc(ServerRef, Config),
    Result = cloudi_x_erlasticsearch:mget_doc(ServerRef, Query),
    ?DOCUMENT_DEPTH  = docs_from_result(Result),
    process_t_delete_doc(ServerRef, Config).
...
t_search(Config) ->
    ...
    lists:foreach(fun(X) ->
                Query = param_query(X),
                Result = cloudi_x_erlasticsearch:search(ServerRef, Index, Type, <<>>, [{q, Query}]),
...
t_count(Config) ->
    ...
    lists:foreach(fun(X) ->
                ...
                % query as parameter
                Result1 = cloudi_x_erlasticsearch:count(ServerRef, Index, Type, <<>>, [{q, Query1}]),
                % The document is structured so that the number of top level
                % keys is as (?DOCUMENT_DEPTH + 1 - X)
                ?DOCUMENT_DEPTH  = count_from_result(Result1) + X - 1,

                % query as doc
                Result2 = cloudi_x_erlasticsearch:count(ServerRef, Index, Type, Query2, []),
                % The document is structured so that the number of top level
                % keys is as (?DOCUMENT_DEPTH + 1 - X)
...
t_delete_by_query_param(Config) ->
    ...
    DResult1 = cloudi_x_erlasticsearch:delete_by_query(ServerRef, Index, Type, <<>>, [{q, Query1}]),
    true = cloudi_x_erlasticsearch:is_200(DResult1),
    cloudi_x_erlasticsearch:flush(ServerRef, Index),
    DResult1a = cloudi_x_erlasticsearch:count(ServerRef, Index, Type, <<>>, [{q, Query1}]),
    0  = count_from_result(DResult1a),

    % All Indices
    process_t_insert_doc(ServerRef, Config),
    ADResult1 = cloudi_x_erlasticsearch:delete_by_query(ServerRef, <<>>, [{q, Query1}]),
    true = cloudi_x_erlasticsearch:is_200(ADResult1),
    cloudi_x_erlasticsearch:flush(ServerRef, Index),
    ADResult1a = cloudi_x_erlasticsearch:count(ServerRef, <<>>, [{q, Query1}]),
...
t_delete_by_query_doc(Config) ->
    ...
    Result1 = cloudi_x_erlasticsearch:count(ServerRef, Index, Type, <<>>, [{q, Query1}]),
    5 = count_from_result(Result1),
    DResult1 = cloudi_x_erlasticsearch:delete_by_query(ServerRef, Index, Type, Query2, []),
    true = cloudi_x_erlasticsearch:is_200(DResult1),
    cloudi_x_erlasticsearch:flush(ServerRef, Index),
    DResult1a = cloudi_x_erlasticsearch:count(ServerRef, Index, Type, <<>>, [{q, Query1}]),
    0  = count_from_result(DResult1a),
%    process_t_delete_doc(ServerRef, Config).
    ...
    % All Indices
    process_t_insert_doc(ServerRef, Config),
    ADResult1 = cloudi_x_erlasticsearch:delete_by_query(ServerRef, Query2),
    true = cloudi_x_erlasticsearch:is_200(ADResult1),
    cloudi_x_erlasticsearch:flush(ServerRef, Index),
    ADResult1a = cloudi_x_erlasticsearch:count(ServerRef, <<>>, [{q, Query1}]),
    0  = count_from_result(ADResult1a).
...
param_query(X) ->
    Key = key(X),
    Value = value(X),
    cloudi_x_erlasticsearch:join([Key, Value], <<":">>).

json_query(X) ->
    Key = key(X),
    Value = value(X),
    cloudi_x_jsx:encode([{<<"query">>, [{<<"term">>, [{Key, Value}]}]}]).
...
t_is_doc(Config) ->
    ...
    lists:foreach(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                true = true_response(cloudi_x_erlasticsearch:is_doc(ServerRef, Index, Type, BX))
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    process_t_delete_doc(ServerRef, Config).
...
t_get_doc(Config) ->
    ...
    lists:foreach(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                Response = cloudi_x_erlasticsearch:get_doc(ServerRef, Index, Type, BX),
                true = cloudi_x_erlasticsearch:is_200(Response)
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    process_t_delete_doc(ServerRef, Config).
...
t_update_doc(Config) ->
    ...
    lists:foreach(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                Doc = ?UPDATE_DOC,
                Response1 = cloudi_x_erlasticsearch:update_doc(ServerRef, Index, Type, BX, Doc),
                true = cloudi_x_erlasticsearch:is_200(Response1),
                Response2 = cloudi_x_erlasticsearch:get_doc(ServerRef, Index, Type, BX),
...
process_t_insert_doc(ServerRef, Config) ->
    ...
    lists:foreach(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                Response = cloudi_x_erlasticsearch:insert_doc(ServerRef, Index, 
                                                     Type, BX, json_document(X)),
                true = cloudi_x_erlasticsearch:is_200_or_201(Response)
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    cloudi_x_erlasticsearch:flush(ServerRef, Index).
...
process_t_bulk(ServerRef, Config) ->
    ...
    Type = ?config(type, Config),
    lists:foreach(fun(X) ->
                Response1 = cloudi_x_erlasticsearch:bulk(ServerRef, <<>>, 
                                                <<>>, bulk_document(<<>>, <<>>, X)),
                true = cloudi_x_erlasticsearch:is_200_or_201(Response1),
                Response2 = cloudi_x_erlasticsearch:bulk(ServerRef, Index, 
                                                Type, bulk_document(Index, <<>>, X)),
                true = cloudi_x_erlasticsearch:is_200_or_201(Response2),
                Response3 = cloudi_x_erlasticsearch:bulk(ServerRef, Index, 
                                               Type, bulk_document(Index, Type, X)),
                true = cloudi_x_erlasticsearch:is_200_or_201(Response3)
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    cloudi_x_erlasticsearch:flush(ServerRef, Index).
...
process_t_delete_doc(ServerRef, Config) ->
    ...
    lists:foreach(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                Response = cloudi_x_erlasticsearch:delete_doc(ServerRef, Index, Type, BX),
                true = cloudi_x_erlasticsearch:is_200(Response)
        end, lists:seq(1, ?DOCUMENT_DEPTH)),
    cloudi_x_erlasticsearch:flush(ServerRef, Index).
...
create_indices(ServerRef, Index) ->
    lists:foreach(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                FullIndex = cloudi_x_erlasticsearch:join([Index, BX], <<"_">>),
                cloudi_x_erlasticsearch:create_index(ServerRef, FullIndex),
                cloudi_x_erlasticsearch:flush(ServerRef, Index)
        end, lists:seq(1, ?DOCUMENT_DEPTH)).
...
are_indices_1(ServerRef, Index) ->
    lists:foreach(fun(X) ->
                BX = list_to_binary(integer_to_list(X)),
                FullIndex = cloudi_x_erlasticsearch:join([Index, BX], <<"_">>),
                true = true_response(cloudi_x_erlasticsearch:is_index(ServerRef, FullIndex))
        end, lists:seq(1, ?DOCUMENT_DEPTH)).
...
are_indices_all(ServerRef, Index) ->
    ...
    true = true_response(cloudi_x_erlasticsearch:is_index(ServerRef, FullIndexList)).
...
delete_this_index(ServerRef, Index) ->
    case true_response(cloudi_x_erlasticsearch:is_index(ServerRef, Index)) of
        true ->
            Response = cloudi_x_erlasticsearch:delete_index(ServerRef, Index),
            true = cloudi_x_erlasticsearch:is_200(Response);
        false -> true
    end.

json_document(N) ->
    cloudi_x_jsx:encode(document(N)).
...
bulk_document(Index, Type, N) ->
    case N of
        ...
        X -> cloudi_x_erlasticsearch:join([bulk_line(Index, Type, X), bulk_document(Index, Type, X-1)], <<>>)
    end.
...
data_index(Data, Index) ->
    ...
    cloudi_x_erlasticsearch:join([BData, BIndex], <<"_">>).
...
setup_lager() ->
    cloudi_x_reltool_util:application_start(cloudi_x_lager),
    cloudi_x_lager:set_loglevel(lager_console_backend, debug),
    cloudi_x_lager:set_loglevel(lager_file_backend, "console.log", debug).

start(Config) ->
    PoolName = ?config(pool, Config),
    ConnectionOptions = ?config(connection_options, Config),
    PoolOptions = ?config(pool_options, Config),
    cloudi_x_reltool_util:application_start(cloudi_x_erlasticsearch),
    cloudi_x_erlasticsearch:start_pool(PoolName, PoolOptions, ConnectionOptions)
    .

stop(Config) ->
    PoolName = ?config(pool, Config),
    cloudi_x_erlasticsearch:stop_pool(PoolName),
    cloudi_x_reltool_util:application_stop(cloudi_x_erlasticsearch),
    ok.
